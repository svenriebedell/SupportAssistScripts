...
<#
.Synopsis
   This PowerShell is for running as remediation script with solutions like Dell TechDirect Remediation or Intune Remediation.
   IMPORTANT: This script need a client installation of Dell SupportAssit for Business 5.2 or later.
   IMPORTANT: This script does not reboot the system to apply or query system.
.DESCRIPTION
   PowerShell script using Dell SupportAssist to update missing drivers with severity level of critical. This script performs detection to check if a client has pending updates with critical level and update these Drivers and reports back to Dell TechDirect Remediation or Intune Remediation if it was successful.
...
$SACLIParameters = '--driverupdate --criticality "Critical" --resultpath'
$ServiceTag = (Get-CimInstance -ClassName Win32_ComputerSystemProduct).IdentifyingNumber
$XMLReportPath = "C:\Temp\DellSA"
$XMLReportFileName = $ServiceTag +"_SupportAssist_DriverInstallResult_*"
...
try
    {
        ...
        try
            {
                ...
                Write-Verbose "Event source SupportAssistRemediation created for log Dell." -Verbose
            }
        catch
            {
                Write-Verbose "Event source SupportAssistRemediation exist." -Verbose
            }
        ...
        if ($null -eq $SACLIExists)
            {
                ...
                try
                    {
                        Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType error -EventId 11 -Message "SupportAssist.exe not found. Please check if SupportAssist is installed on this device."
                    }
                catch
                    {
        ...
        if ($null -eq $DSAInstalled)
            {
                ...
                try
                    {
                        Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType error -EventId 11 -Message "SupportAssist Version 5.2 or higher is not installed."
                    }
                catch
                    {
                        Write-Verbose "EventLog write failed"
        ...
        if (-not $DirectoryCreated)
            {
                ...
                try
                    {
                        Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType error -EventId 11 -Message "Report Path creation failed."
                    }
                catch
                    {
                        Write-Verbose "EventLog write failed"
        ...
        try
            {
                Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType Information -EventId $exitCode -Message "DSA driverscan completed with exit code: $exitCode"
            }
        catch
            {
                Write-Verbose "EventLog write failed"
        ...
        If ($null -ne $availableXML)
                ...
                if ($fileAge.TotalMinutes -gt 5)
                    {
                        ...
                        try
                            {
                                Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType Information -EventId 10 -Message "XML file is older than 5 minutes from exit time."
                            }
                        catch
                            {
                                Write-Verbose "EventLog write failed"
                        ...
                        try
                            {
                                ...
                                $InstsllResult = $xmlContent.DriverInstallResults | Select-Object -ExpandProperty OverallStatus
                            }
                        catch
                            {
                        ...
                        If($type -eq "Information")
                            {
                                ...
                                try
                                    {
                                        Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType $type -EventId $exitCode -Message $message
                                    }
                                catch
                                    {
                                        Write-Verbose "EventLog write failed"
                                ...
                                Write-Output "~~Critical Drivers successfully installed~~"
                                Write-Output "|Update successfull|"
                                Exit 0
                            }
                        else
                            {
                    ...
                    If($type -eq "Information")
                        {
                            ...
                            try
                                {
                                    $message += "`n" + ($xmlContent.DriverInstallResults | Out-String)
                                    Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType $type -EventId $exitCode -Message $message
                                }
                            catch
                                {
                            ...
                            $Fullmessage = $Fullmessage = "~~Critical Driver update not found: " + ($message -replace '\r?\n', '') + "~~"
                            Write-Output "$Fullmessage"
                            Write-Output "|Critical update not found|"
                            Exit 0
                        }
                    else
                        {
        ...
        else
            {
                ...
                try
                    {
                        Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType Information -EventId 10 -Message "XML report not found."
                    }
                catch
                    {
                        Write-Verbose "EventLog write failed"
...
catch
    {
        ...
        try
            {
                Write-EventLog -LogName Dell -Source "SupportAssistRemediation" -EntryType error -EventId 11 -Message "Script Error $($_.Exception.Message)"
            }
        catch
            {
                Write-Verbose "EventLog write failed"
